...
- **Process detection portability.** macOS uses sysctl `kern.proc.all` for pid enumeration plus `proc_pidinfo(PROC_PIDVNODEPATHINFO)` and `sysctl(KERN_PROCARGS2)` per pid, all via raw `syscall.Syscall6` and `unix.SysctlRaw` — no cgo. (`proc_listpids` is restricted on macOS 26+ for unsigned binaries; sysctl works without entitlements.) Linux walks `/proc/*/cwd`. Both platforms go through a single `enumerate(ctx)` seam so `procs/procs.go` owns the prefix filter.
